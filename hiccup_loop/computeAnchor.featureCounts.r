...
counts[,-c(1:6)] = sweep(counts[,-c(1:6)],2,sums,'/')*1e6
m = merge(anchor,counts, by.x="V7",by.y="Geneid",all.x=T)
m[,c(13:16)][is.na(m[,c(13:16)])] = 0
agg = aggregate(.~V1+V2+V3, m[,c(2:4,13:16)],sum)
colnames(agg) = c("chr","start","end",paste0(tf,c("_D00","_D02","_D05","_D15")))
write.table(agg,paste0("overlap_anchors_to_features/anchor.",tf,".norm_counts.txt"),quote=F,sep='\t',row.names=F)
