...
for chr in {3..1} X; do
  ...
  loop=../../analysis/hiccup_loops/loops_merged_across_samples.tab
...
head -n 1 $outdir/loops_merged.1.mat.txt |awk -v OFS="\t" '{print "chr",$0}' > $outdir/../loops_merged.mat.txt
head -n 1 $outdir/loops_merged.1.oe.txt |awk -v OFS="\t" '{print "chr",$0}' > $outdir/../loops_merged.oe.txt
for chr in {1..22} X; do
   awk -v OFS="\t" -v chr=$chr '{ if(NR>1) print  "chr"chr,$0}' $outdir/loops_merged.${chr}.mat.txt >>  $outdir/../loops_merged.mat.txt
