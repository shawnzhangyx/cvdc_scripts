...
pdf("figures/arrowhead_TAD_number.pdf",width=3,height=3)
ggplot(dat,aes(x=names,y=num)) +
  geom_line(aes(group=1),color='grey') +
  geom_point() + ylim(5000,8000) + 
  #geom_bar(stat="identity",fill=cbbPalette[6],width=0.75) +
...
ggplot(dist, aes(x=names,y=dist/1000)) +
#  geom_violin() +
