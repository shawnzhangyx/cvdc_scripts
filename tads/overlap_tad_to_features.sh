...
for name in D00.unique D00.pre D00.pos; do 

echo -e "chr\tstart\tend\tCTCF\tATAC\tH3K4me1\tH3K4me3\tH3K27me3\tH3K27ac\tTSS" |tee \
$outdir/${name}.all_features.txt

intersectBed -a ${name}.tads -b ../../data/tfChIPseq/merged_peaks/CTCF_merged_peaks.bed -c |
...
intersectBed -b ../../data/atac/peaks/atac_merged_peaks.bed -a ${name}.tads -loj > $outdir/${name}.atac_merged_peaks.txt
intersectBed -b ../../data/tfChIPseq/merged_peaks/CTCF_merged_peaks.bed -a ${name}.tads -loj > $outdir/${name}.CTCF_merged_peaks.txt

for mark in H3K4me1 H3K4me3 H3K27ac H3K27me3; do
intersectBed -b $chipdir/${mark}_merged_peaks.bed -a ${name}.tads -loj > $outdir/${name}.${mark}_merged_peaks.txt
done
intersectBed -b ../../data/annotation/gencode.v19.annotation.transcripts.tss1k.bed -a ${name}.tads -loj > $outdir/${name}.gene_tss.txt
