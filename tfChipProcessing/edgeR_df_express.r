...
colnames(counts) = paste(rep(c("D00","D02","D05","D07","D15","D80"),each=2), rep(c(1,2),6),sep='_')[1:11]
...
bonf = p.adjust(lrt$table$PValue,method="bonferroni")
out = cbind(tab[keep,1:6],cpm(y),lrt$table,qBH,bonf)
out[,c(7:17)] = sweep(out[,c(7:17)],1,out$Length,'/')*1000
