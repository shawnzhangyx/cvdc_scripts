...
tail -n +2  ../../analysis/hiccup_loops/loops_merged_across_samples.uniq.tab |
while read chr x1 x2 field4; do
  contact=$(../utility/straw NONE $sample $chr:$x1:$x1 $chr:$x2:$x2 BP 10000 |cut -f 3)
  if [ -z "$contact" ]; then contact=0;fi 
  echo -e "$chr\t$x1\t$x2\t$contact" \
  >> ../../analysis/hiccup_loops/contacts_by_samples/$name.contacts.10k.txt
    done &
## 30kb
...
echo -e "chr\tx1\ty1\tD00_1\tD00_2\tD02_1\tD02_2\tD05_1\tD05_2\tD07_1\tD07_2\tD15_1\tD15_2\tD80_1\tD80_2" |tee loops_merged.uniq.10k.mat.txt loops_merged.uniq.30k.mat.txt
paste contacts_by_samples/*.contacts.10k.txt | \
  cut -f 1,2,3,$(seq -s, 4 4 $((4*12)) ) | tail -n +2 >> loops_merged.uniq.10k.mat.txt
