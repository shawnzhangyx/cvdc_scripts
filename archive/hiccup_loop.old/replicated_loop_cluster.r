...
d = read.delim("replicated_loops.cpb.logFC.edger.dynamic.txt")
...
pdf("figures/replicated_dynamic_loops.k_2to10.pdf")
for(k in 2:10){
print(k)
...
pdf("figures/replicated_dynamic_loops.kmeans_K6.pdf")
heatmap.2(km$centers,Colv=FALSE,Rowv=FALSE,
dendrogram="none",cexRow=1,cexCol=1,notecol='black',margins=c(5,5),tracecol=F,
col=colorRampPalette(c("lightyellow","red"))
)
...
write.table(d,"replicated_loops/loops.cpb.logFC.edger.dynamic.cluster.txt",quote=F,sep='\t')
