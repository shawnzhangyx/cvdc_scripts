...
loop.tad = read.delim("loop_to_tad/loops.tad.txt")
loop$a1 = sub("(.*) (.*) (.*)","\\1 \\2",loop$name)
loop$a2 = sub("(.*) (.*) (.*)","\\1 \\3",loop$name)
loop$tad = loop.tad$tad[match(loop.tad$name,loop$name)]
...
print(mark)
rna1.test[[mark]] = data.frame(tad=p1$tad, cor =sapply(1:nrow(rna1), function(i){
  cor.test(as.numeric(rna1[i,]), as.numeric(test1[i,grep(mark,colnames(test1))]))$estimate }) ) 
rna1.control[[mark]] = data.frame(tad=p1$tad, cor = sapply(1:nrow(rna1), function(i){
...
for (mark in c("H3K27ac","H3K4me1","H3K27me3")) {
print(mark)
rna2.test[[mark]] = data.frame(tad=p2$tad, cor =sapply(1:nrow(rna2), function(i){
  cor.test(as.numeric(rna2[i,]), as.numeric(test2[i,grep(mark,colnames(test2))]))$estimate }) )
rna2.control[[mark]] = data.frame(tad=p2$tad, cor = sapply(1:nrow(rna2), function(i){
...
#    ifelse(combined.stats$tad>0, "Loop-Dynamic","Loop-Static"))
#combined.stats$type2 = factor(combined.stats$type2,levels=c("Loop-Dynamic","Loop-Static",
#  "Control"))

pdf("figures/anchor.mark.correlation.by_tad.pdf",height=3,width=6)
ggplot(combined.stats, aes(x=mark, fill=tad,color=type, y=cor)) + 
