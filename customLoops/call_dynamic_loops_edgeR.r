...
o=read.delim("combined_loops.uniq.counts.txt")
...
write.table(out,"edgeR/loops_edgeR_test_allStage.txt",row.names=F,sep='\t',quote=F)
...
for (i in 1:length(myContrasts)){
  ...
  fdr = p.adjust(lrt$table$PValue,method="bonferroni")
  out = cbind(o[,1:3],lrt$table,fdr)
  write.table(out,paste0("edgeR/loops_edgeR_test_",myContrasts[i],".txt"),row.names=F,sep='\t',quote=F)

  #write.table(out[order(out$fdr),],paste0("edgeR/loops_edgeR_test_",myContrasts[i],".txt"),row.names=F,sep='\t',quote=F)
}

write.table(cpm(y)*1000, "edgeR/loops_cpb.txt",sep="\t",quote=F)
