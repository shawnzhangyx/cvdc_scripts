...
print(mark)
anchor = read.delim(paste0("loop_control_distance_matched/anchor.",mark,"_merged_peaks.txt"),header=F)
...
write.table(ave,paste0("loop_control_distance_matched/anchor.",mark,".norm_counts.txt"),quote=F,sep='\t',row.names=F)
}

## CTCF. Needs to clean up. 
tf =  "CTCF"
anchor = read.delim(paste0("loop_control_distance_matched/anchor.",tf,"_merged_peaks.txt"),header=F)
...
write.table(ave,paste0("loop_control_distance_matched/anchor.",tf,".norm_counts.txt"),quote=F,sep='\t',row.names=F)


#mark = "ATAC"
## RNA-seq 
anchor = read.delim("loop_control_distance_matched/anchor.gene_tss.unique.txt",header=F)
...
write.table(ave,"loop_control_distance_matched/anchor.rnaseq.norm_counts.txt",quote=F,sep='\t',row.names=F)
